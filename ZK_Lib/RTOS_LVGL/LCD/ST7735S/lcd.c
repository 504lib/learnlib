...
uint16_t dma_buffer1[128];  // 双缓冲
uint16_t dma_buffer2[128];
...
void LCD_WR_DATA(uint16_t dat)
{
    LCD_CS_Clr();
    // 改为分两次发送，确保字节顺序正确
    HAL_SPI_Transmit_DMA(&hspi1, (uint8_t*)&dat + 1, 1);  // 高字节
	while(HAL_SPI_GetState(&hspi1) != HAL_SPI_STATE_READY);
    HAL_SPI_Transmit_DMA(&hspi1, (uint8_t*)&dat, 1);      // 低字节
	while(HAL_SPI_GetState(&hspi1) != HAL_SPI_STATE_READY);
    LCD_CS_Set();
}
...
{          
    ...
    for(uint32_t i = 0; i < count; i++) {
        HAL_SPI_Transmit(&hspi1, &color_high, 1, 100);
        HAL_SPI_Transmit(&hspi1, &color_low, 1, 100);
    }
...
void LCD_ShowChar_DMA(uint16_t x, uint16_t y, uint8_t num, uint16_t fc, uint16_t bc, uint8_t sizey, uint8_t mode)
{
    ...
    // 创建行缓冲区
    #define CHAR_LINE_BUF_SIZE 32
    uint16_t line_buffer[CHAR_LINE_BUF_SIZE];
    uint16_t line_pos = 0;
    ...
    for(i = 0; i < TypefaceNum; i++)
    {
        ...
        for(t = 0; t < 8; t++)
        {
            ...
            {
                // 填充行缓冲区
                line_buffer[line_pos++] = (temp & (0x01 << t)) ? fc : bc;
                
                // 缓冲区满或行结束时发送
                if(line_pos >= CHAR_LINE_BUF_SIZE || (i * 8 + t + 1) % sizex == 0)
                {
                    if(line_pos > 0)
                    {
                        HAL_SPI_Transmit(&hspi1, (uint8_t*)line_buffer, line_pos * 2, 100);
                        line_pos = 0;
                    }
                }
            }
