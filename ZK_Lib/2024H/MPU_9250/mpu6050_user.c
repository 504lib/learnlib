...
void MPU6050_Update(void) {
    ...
    // LOG_DEBUG("Raw Gyro: gx=%d, gy=%d, gz=%d", mpu_data.raw.gx, mpu_data.raw.gy, mpu_data.raw.gz);
    // LOG_DEBUG("Physical Gyro: gx=%.2f rad/s, gy=%.2f rad/s, gz=%.2f rad/s", mpu_data.phys.gx, mpu_data.phys.gy, mpu_data.phys.gz);
...
void MPU6050_Calibrate(uint16_t samples) {
    ...
    LOG_DEBUG("Calibration complete: ax_offset=%.2f, ay_offset=%.2f, az_offset=%.2f, gx_offset=%.2f, gy_offset=%.2f, gz_offset=%.2f",
              mpu_data.calib.ax_offset, mpu_data.calib.ay_offset, mpu_data.calib.az_offset,
              mpu_data.calib.gx_offset, mpu_data.calib.gy_offset, mpu_data.calib.gz_offset);
	OLED_Clear();
    OLED_ShowString(0, 0, (uint8_t*)"Calibration", 16,1);
    OLED_ShowString(0, 16, (uint8_t*)"Complete!", 16,1);
    OLED_Refresh();
    HAL_Delay(1000);
    OLED_Clear();
}
