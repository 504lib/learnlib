...
#define sampleFreqDef	50.0f		// default sample frequency in Hz
...
static float invSampleFreq = 1.0f / sampleFreqDef;
...
void MadgwickAHRSsetSampleFreq(float frequency_hz) {
	if (frequency_hz > 0.0f) {
		invSampleFreq = 1.0f / frequency_hz;
	}
}
...
void MadgwickAHRSupdate(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz) {
	...
	if(!((ax == 0.0f) && (ay == 0.0f) && (az == 0.0f))) {
		...
		if (stepNormSq > 0.0f) {
			recipNorm = invSqrt(stepNormSq); // normalise step magnitude
			s0 *= recipNorm;
			s1 *= recipNorm;
			s2 *= recipNorm;
			s3 *= recipNorm;
