...
uint8_t dma_buffer1[LCD_WIDTH * 2];  // 双缓冲
uint8_t dma_buffer2[LCD_WIDTH * 2];
...
void LCD_Clear(uint16_t color)
{
    ...
    {
        dma_buffer1[i] = color_high;
        dma_buffer1[i + 1] = color_low;
        dma_buffer2[i] = color_high;
        dma_buffer2[i + 1] = color_low;
    }   
    ...
    for(uint32_t y = 0; y < height; y++) 
	{
        ...
        if(current_buffer == 0) 
		{
            HAL_SPI_Transmit_DMA(&hspi1, dma_buffer1, LCD_WIDTH * 2);
            current_buffer = 1;
        } 
		else 
		{
            HAL_SPI_Transmit_DMA(&hspi1, dma_buffer2, LCD_WIDTH * 2);
            current_buffer = 0;
        }
    }
...
// {
//     // 使用映射修复
//     uint16_t fixed_color = mapped_color_fix(color);
...
//     // 使用阻塞发送填充整个屏幕
//     uint32_t total_pixels = lcddev.width * lcddev.height;
//     uint8_t color_high = fixed_color >> 8;
//     uint8_t color_low = fixed_color & 0xFF;
