...
void protocol:: Send_Uart_Frame_WEIGHT(float weight_value)
{
    typedef union
    {
        float value;
    ...
    frame[2] = static_cast<uint8_t>(CmdType::HX711_WEIGHT);
    frame[3] = 5;
...
void protocol::Send_Uart_Frame_CURRENT_USER(String name)
{
    ...
    // 帧头
    frame[index++] = Headerframe1;
    frame[index++] = Headerframe2;
    ...
    // 帧尾
    frame[index++] = Tailframe1;
    frame[index++] = Tailframe2;
        ...
        case WAIT_TAIL2:
            if(data == Tailframe2)
            {
                ...
                if(calc_check == recv_check)
                {
                    ...
                    else if (frame_type == static_cast<uint8_t>(CmdType::HX711_WEIGHT) && frame_len == 4)
                    {
                        Medicine type;
                        union {
                            float f;
...
void protocol::setWeightCallback(HX711_WEIGHT_Callback cb)
{
    weight_callback = cb;
}
