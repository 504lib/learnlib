...
public:
    ...
    {
        if (status == VehicleStatus::ARRIVING)
        {
            Change_Status_Tip(Bus_status::STATUS_ARRIVING);
        }
        else if (status == VehicleStatus::LEAVING)
        {
            Change_Status_Tip(Bus_status::STATUS_LEAVING);
        }
...
{
    router_list.Add_Station_to_Tail("normal_univercity","师范学院",target_ssid,target_password,target_station_server);
    for(;;)
    {
        router_list.Scheduler();
...
void setup() 
{
    ...
    uart_protocol.setVehicleStatusCallback([](VehicleStatus status){
        if (status == VehicleStatus::ARRIVING)
        {
            Serial.printf("Vehicle Status: ARRIVING\n");
            router_list.Change_Status_Tip(status);
        }
        else if (status == VehicleStatus::LEAVING)
        {
            Serial.printf("Vehicle Status: LEAVING\n");
            router_list.Change_Status_Tip(status);
        }
        else if (status == VehicleStatus::WAITING)
        {
            Serial.printf("Vehicle Status: WAITING\n");
            router_list.Change_Status_Tip(status);
        }
    });
