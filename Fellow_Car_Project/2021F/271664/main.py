# main.py
from maix import camera, display, image, nn, app   # 删除 comm
import serial            # 新增
import struct, os, time  # 保持不变
...
def init_hardware():
    ...
    dis = display.Display()
    # 修改点：使用 serial.Serial
    try:
        uart = serial.Serial(UART_DEVICE, baudrate=UART_BAUD, timeout=0.01)
...
def try_read_frame():
    ...
    if uart is None or uart.in_waiting < 8:   # 修改点：any() -> in_waiting
        return None, None
    data = uart.read(uart.in_waiting)         # 修改点：读取全部可用数据
