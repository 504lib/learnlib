...
//ctrl_mode = CTRL_MODE_TRACKING;    // 设置灰度循迹模式 
  KeyControl_Init();
  Tasks_Init();
  App_FSM_Init();              // HSM 状态机初始化
...
//  SM_StartTask(TASK_4);
  ...
  while (1)
  {
	  ...
	  App_Protocol_Run();          // 驱动协议状态机，处理蓝牙接收
      App_FSM_Run();           // 运行状态机任务
	  // 获取显示模式
...
//	  task2(&task2_pt);
	  ...
	  OLED_Task(&oled_pt); 
    // LOG_INFO("rx_count: %lu,current char: %c", rx_count, uart1_rx_byte); // 定期打印接收计数器，验证中断是否正常工作
    // HAL_Delay(100); // 避免打印过快导致串口输出混乱
    /* USER CODE END WHILE */
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    {
        rx_count++;
        Uart_Protocol_ProcessReceivedData8bit(App_GetProtocolInstance_Cam(), uart2_rx_byte);
        HAL_UART_Receive_IT(&huart2, &uart2_rx_byte, 1);
    }
    else if (huart->Instance == USART3)   // 蓝牙 → 独立协议实例
    {
        Uart_Protocol_ProcessReceivedData8bit(App_GetProtocolInstance_Bt(), uart3_rx_byte);
