...
static void convert_to_physical(void) {
    const float ACCEL_SCALE = 16384.0f;  // ±2g (FS_SEL=0)
    const float GYRO_SCALE  = 16.4f;     // ±2000dps (FS_SEL=3)
...
void MPU6050_Calibrate(uint16_t samples) {
    ...
    for (uint16_t i = 0; i < samples; i++) {
        ...
        uint32_t elapsed = DL_GetTick() - t_start;
        snprintf(buffer,sizeof(buffer),"%lums %d/%d", elapsed, i+1, samples);
