...
#define RAMP_ACCEL  0.08f
#define RAMP_DECEL  0.12f
#define RAMP_STOP_THRESHOLD  0.02f
static float ramp_speed = 0.0f;
static bool  ramp_enabled = true;
static float decel_at_dist = 0.0f;
...
void Control_Update(float dt)
{
    ...
    case CTRL_MODE_DECEL:
        if (ramp_enabled) {
            if (ramp_speed > 0.0f) {
                ramp_speed -= RAMP_DECEL * dt_s;
                if (ramp_speed < 0.0f) ramp_speed = 0.0f;
            }
            if (ramp_speed < RAMP_STOP_THRESHOLD)
                { ramp_speed = 0.0f; ctrl_mode = CTRL_MODE_STOP; }
        } else {
            ramp_speed = 0.0f; ctrl_mode = CTRL_MODE_STOP;
        }
        break;
...
/* ---- 斜坡开关 ---- */
void Control_SetRampEnabled(bool en) { ramp_enabled = en; }
...
{
    if (ramp_enabled && ramp_speed > RAMP_STOP_THRESHOLD) {
        ctrl_mode = CTRL_MODE_DECEL;
    } else {
...
/* ---- 设减速点: target_distance 为总路程, 根据当前速度算提前量 ---- */
void Control_SetDecelDistance(float target_distance) {
    float decel_len = (base_speed * base_speed) / (2.0f * RAMP_DECEL);
    decel_at_dist = (target_distance > decel_len) ? (target_distance - decel_len) : 0.0f;
}

/* ---- 周期性检查: 当前路程是否该减速了 ---- */
void Control_CheckDecel(float current_dist) {
    if (ramp_enabled && ctrl_mode == CTRL_MODE_RUN && current_dist >= decel_at_dist) {
        Control_Stop();  // 进 DECEL 模式
    }
}
